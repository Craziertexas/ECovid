...
function RegisterAdd() {
  ...
  var [oldPatientID, setOldPatientID] = useState() ;
  ...
  async function updateForm() {
    patient = await getPatient(history.location.state['PATIENTID']);
    convert(patient);
  }
  ...
  async function getPatient(patientID) {
    ...
    return new Promise(async function(resolve, reject) {
      ...
      await axios.post('http://localhost:4000/Register/getPatient',JSON.stringify(params),
      {
  ...
  function convert(selectPatient) {

    selectPatient = selectPatient[0];
    console.log(selectPatient);
    var buffpatient = {
      name: selectPatient['NAME'],
      lastname: selectPatient['LASTNAME'],
      id: selectPatient['ID'].toString(),
      sex: { value: selectPatient['SEX'], label: selectPatient['SEX']},
      birthday: formatTimeStamp(selectPatient['BIRTHDAY']),
      homeloc: {lat: selectPatient['HOMELOCLAT'], lng: selectPatient['HOMELOCLONG']},
      workloc: {lat: selectPatient['WORKLOCLAT'], lng: selectPatient['WORKLOCLONG']},
      exam: { value: selectPatient['EXAM'], label: selectPatient['EXAM']},
      examdate: formatTimeStamp(selectPatient['EXAMDATE']),
      patientid: selectPatient['PATIENTID'],
    }

    setOldPatientID(selectPatient['PATIENTID']);
    setPatient(buffpatient);
  ...
  async function onSubmit() {
    ...
    if (!checkDuplicate(patients) && !checkEmpty()) {
      ...
      params = {
        ...
        "OLDPATIENTID": await crypter.encrypt(oldPatientID.toString())
      }
      console.log(JSON.stringify(params));
      sessionCheck.SessionCheck('assistantUser','/Register',history.push);
      await axios.post('http://localhost:4000/Register/editPatient',JSON.stringify(params),
      {
        "headers":{"content-type":"application/json"}
      })
      .then((res) => {
